...
const OnboardingTour = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    // Check local storage only on mount if not explicitly opened via help button
    if (!isOpen) {
      const completed = localStorage.getItem('sky-portal-tutorial-completed');
      if (!completed) {
  ...
  const handleComplete = () => {
    localStorage.setItem('sky-portal-tutorial-completed', 'true');
    setIsCompleted(true);
    if (onClose) onClose();
  };
    ...
    <div className={styles.modalOverlay}>
      {stepIndex === -1 ? (
        <div className={styles.welcomeCard}>
          <h2 className={styles.welcomeTitle}>Welcome to Sky Portal</h2>
