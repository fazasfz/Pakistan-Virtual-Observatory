...
export const fetchAPOD = async () => {
  try {
    const response = await axios.get(`${BASE_URL}/apod`, { timeout: 8000 });
    if (response.data && (response.data.url || response.data.hdurl)) {
      return response.data;
    }
  } catch (error) {
    console.warn('Backend APOD fetch warning, serving local astronomical fallback:', error);
  }
